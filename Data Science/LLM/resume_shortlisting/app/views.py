...
from .custom_email import send_application_confirmation
...
def jobdetails(request, id):
    ...
    applied_job = JobApplication.objects.filter(job=job).order_by('-ats_score')[:10]
    print(applied_job)
...
def apply_job(request):
    if request.method == "POST":
        job_post = JobPost.objects.get(id=request.POST.get("job_id"))
        ...
        # Call ATS with correct argument order
        ats = ats_score(
