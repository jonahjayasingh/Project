...
export function BubbleSort() {
  ...
  const startSort = () => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      if (j < arr.length - i - 1) {
        setStatus(`Comparing index ${j} and ${j + 1}: ${arr[j]} & ${arr[j + 1]}`);
    ...
    return (
      ...
      >
        <Text
          style={[
            styles.boxText,
            (isCompared || isSwapping || isSorted) && styles.boxTextActive,
          ]}
        >
          {item}
        </Text>
        <Text style={styles.indexText}>{index}</Text>
        {isCompared && !isSwapping && <Text style={styles.labelText}>comparing</Text>}
