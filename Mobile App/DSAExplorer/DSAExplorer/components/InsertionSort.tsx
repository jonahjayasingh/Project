...
export function InsertionSort() {
  ...
  const [comparingIndices, setComparingIndices] = useState<{ first: number; second: number } | null>(null);
  ...
  const startSort = () => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      switch (state.currentStep) {
        case 'comparing':
          // Set current indices for visualization
          setCurrentIndices({ i: state.i, j: state.j });
          
          if (state.j > 0) {
            // Show comparing state
            setComparingIndices({ first: state.j, second: state.j - 1 });
            setStatus(`Comparing ${state.arr[state.j]} and ${state.arr[state.j - 1]}`);
            ...
            // j reached beginning, move to next i
            state.currentStep = 'moving';
          }
          break;
        ...
        case 'swapping':
          ...
          setStatus(`Swapping ${state.arr[state.j]} and ${state.arr[state.j - 1]}`);
          
          // Perform the swap
          [state.arr[state.j], state.arr[state.j - 1]] = [state.arr[state.j - 1], state.arr[state.j]];
          setArray([...state.arr]);
          
          // Move j backwards after swap
          state.j--;
          
          // After swap animation, go back to comparing
          setTimeout(() => {
            setSwappingIndices(null);
        ...
        case 'moving':
          ...
          if (state.i < state.arr.length) {
            setStatus(`Moving to next element at index ${state.i}`);
            state.currentStep = 'comparing';
          }
          break;
      }
  ...
  const renderItem = ({ item, index }: { item: number; index: number }) => {
    ...
    const isComparing = comparingIndices !== null && 
                       (index === comparingIndices.first || index === comparingIndices.second);
    ...
    return (
      <View
        style={[
          styles.box,
          isI && !isComparing && !isSwapping && styles.currentBox,
          isJ && !isComparing && !isSwapping && styles.compareBox,
          isComparing && styles.comparingBox,
          isSwapping && styles.swappingBox,
          isSorted && !isComparing && !isSwapping && styles.sortedBox,
        ]}
      >
        <Text
          style={[
            styles.boxText,
            (isI || isJ || isComparing || isSwapping || isSorted) && styles.boxTextActive,
          ]}
        >
          {item}
        </Text>
        <Text style={styles.indexText}>{index}</Text>
        {isI && !isComparing && !isSwapping && <Text style={styles.labelText}>i</Text>}
        {isJ && !isI && !isComparing && !isSwapping && <Text style={styles.labelText}>j</Text>}
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.infoCard}>
          ...
          <Text style={styles.infoText}>
            ...
            • 🔄 Works by inserting elements into their correct position in sorted portion{'\n'}
        ...
        <View style={styles.legendCard}>
          ...
          <View style={styles.legendItem}>
            ...
            <Text style={styles.legendText}>Current j pointer</Text>
          </View>
          <View style={styles.legendItem}>
            <View style={[styles.legendColor, styles.comparingBox]} />
