...
from .crud_website_details import *
import mimetypes


def send_applicant_email(applicant):
    """
    Sends an email with applicant details.
    """
    subject = f"New Application from {applicant.first_name} {applicant.last_name}"
    body = f"""
    ...
    email = EmailMessage(
        subject=subject,
        body=body,
        from_email=settings.EMAIL_HOST_USER,   # replace with your HR email
        to=[settings.EMAIL_RECIVER],         # HR recipient
        reply_to=[applicant.email],        # optional, to reply directly to applicant
    )

    # Attach resume if needed
    if applicant.resume:
        # Guess MIME type (pdf/docx/etc.)
        mime_type, _ = mimetypes.guess_type(applicant.resume.path)
        mime_type = mime_type or "application/octet-stream"

        with open(applicant.resume.path, "rb") as f:
            email.attach(applicant.resume.name, f.read(), mime_type)
    print("send")
    email.send()
...
def home(request):
    content = {
        "courses":Website_Course.objects.all(),
        "placements":website_Placement.objects.all(),
        "gallery":Website_Gallery.objects.all(),
        "contact": company_contacts.objects.first(),
...
def verify_certificate(request):
    if request.method == "POST":
        id = request.POST.get("certificate_id")
        certificate = get_object_or_404(Student,certificate_id=id)
        if certificate:
            ...
            return redirect("app:verify_certificate")
    content = {
        "contact": company_contacts.objects.first(),
    }
    return render(request,"verify_certificate.html",content)

def services(request):
    content = {
        "services":website_services.objects.all(),
        "contact": company_contacts.objects.first(),
        "domains": website_project_domain.objects.all(),
        "projects":website_project.objects.all(),
    }
...
def careers(request):
    if request.method == "POST":
        ...
        if  company_applicant.objects.filter(email=email,career=career).exists() or company_applicant.objects.filter(phone=phone,career=career).exists():
            print(email,phone,career)
            messages.error(request, "You have already applied")
            return redirect("app:careers")
        applicant = company_applicant.objects.create(
        ...
        send_applicant_email(applicant)
        applicant.save()
        messages.success(request,"Your application has been submitted")
        return redirect("app:careers")
    content = {
        "contact": company_contacts.objects.first(),
        "careers": company_career.objects.all()
    }
    return render(request,"career.html",content)
...
@login_required
def genrate_receipt(request, id):
    ...
    receipt_data = generate_receipt(receipt_data)
    return redirect("app:show_receipt", location=receipt.id)


@login_required
def show_receipt(request, location):
    try:
        receipt = get_object_or_404(Receipt, id=int(location))
    except:
        receipt = False
    if receipt:
        location = f"{receipt.name}_{receipt.course_name}_{receipt.student.id}.pdf".replace(" ","")
...
def single_student(request,student_id):
    ...
    receipt = Receipt.objects.filter(name=student.name)
    if Payment.objects.filter(user=student).exists():
        payment = Payment.objects.get(user=student)
    else:
        payment = None
    return render(request,"single_student.html",{"student":student,"faculty":faculty,"payment":payment,"courses":course,"receipt":receipt})
...
def website_details(request):
    if request.method == "POST":
        ...
        if op == "website_course_create":
            return website_course_create(request)
        elif op == "website_gallery_create":
            return website_gallery_create(request)
        elif op == "website_placement_create":
            return website_placement_create(request)
        elif op == "website_service_create":
            return website_service_create(request)  
        elif op == "website_domain_create":
            return website_domain_create(request)
        elif op == "website_project_create":
            return website_project_create(request)
        elif op == "website_contact_create":
            return website_contact_create(request)
        elif op == "website_career_create":
            return website_career_create(request)
        else:
    ...
    return render(request, "Webiste Details/main.html",content)

@login_required
def delete_website(request):
    if request.method == "POST":
        op = request.POST.get("op")
        if op == "website_course_delete":
            return website_course_delete(request)
        elif op == "website_gallery_delete":
            return website_gallery_delete(request)
        elif op == "website_placement_delete":
            return website_placement_delete(request)
        elif op == "website_service_delete":
            return website_service_delete(request)
        elif op == "website_domain_delete":
            return website_domain_delete(request)
        elif op == "website_project_delete":
            return website_project_delete(request)
        elif op =="website_career_delete":
            return website_career_delete(request)
        else:
            messages.error(request,"Invalid Operation")
            return redirect("app:website_details")
    else:
...
def applicants(request,id):
    content = {
        "applicants": company_applicant.objects.filter(career__id=id),
        "job": company_career.objects.get(id=id)
    }
    return render(request, "Webiste Details/applicants.html",content)

@login_required
def export_applicants(request, id):
    ...
    headers = [
        ...
        "Experience", "Current Company", "Resume",
        "Cover Letter"
    ]
    ...
    # Fetch applicants and add rows
    for applicant in company_applicant.objects.filter(career__id=id):
        row = [
            ...
            applicant.current_company if applicant.current_company else "",
            "",  # Placeholder for Resume hyperlink
            applicant.cover_letter if applicant.cover_letter else ""
        ]
        worksheet.append(row)

        # Add Resume hyperlink
        if applicant.resume:
            resume_view_link = server_url + applicant.resume.url if applicant.resume else ""
            cell = worksheet.cell(row=worksheet.max_row, column=7, value="View Resume")
            cell.hyperlink = resume_view_link
            cell.style = "Hyperlink"
        else:
            worksheet.cell(row=worksheet.max_row, column=7, value="No Resume")
