...
class Domain(models.Model):
    ...
    student_reps = models.ManyToManyField(User, related_name='rep_domains', limit_choices_to={'role': 'rep'}, blank=True)
...
class TopicRequest(models.Model):
    ...
    student = models.ForeignKey(User, on_delete=models.CASCADE, related_name='topic_requests', limit_choices_to={'role': 'student'})
    topic_name = models.CharField(max_length=255)
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='PENDING')
    requested_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"{self.student.username}: {self.topic_name}"
