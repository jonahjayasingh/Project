...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # --- Seed Events ---
        for i in range(200):
            event = ["Exibition","Community outreach"]
            event_name = fake.catch_phrase()
            choice = random.choice(event) if random.random() < 0.7 else None
            event_mcq = (
                random.choice(Mcq.objects.all()) 
                if choice in ["Exibition", "Community outreach"] and random.random() >= 0.8 
                else None
            )
